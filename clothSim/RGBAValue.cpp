...
// contructor from pointer
RGBAValue::RGBAValue(const float *values)
{
  //std::cout << values[0] << " " << values[1] << '\n';
  red   = (float)(values[0] * 255.);
  green = (float)(values[1] * 255.);
  blue  = (float)(values[2] * 255.);
...
// lerp function to interpolate between this colour and another
RGBAValue RGBAValue::lerp(const RGBAValue &other, const float a) const
{ //lerp
  return RGBAValue(   ((1 - a) * red  ) + (a * other.red  ),
                      ((1 - a) * green) + (a * other.green),
                      ((1 - a) * blue ) + (a * other.blue ),
                      ((1 - a) * alpha) + (a * other.alpha));
} // lerp
