...
RGBAValue RGBAImage::getVal(float u, float v)
{ // getVal()
  // scale coords
  int row = int((u * 2.) * height);
  int col = int((v * .5) * width);

  //std::cout << row << " " << col << '\n';

  if(row < 0 || row > (width -1) || col < 0 || col > (height -1))
  {
    return RGBAValue((float)0., (float)0., (float)0., (float)1.);
  }

  RGBAValue *rows = block+(col*width);
  ...
  //return (block+(row*width))[col];

  // return value from texture
